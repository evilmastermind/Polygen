...
If you opt into `compilePolicy: { preset: "compat" }`, warnings can also indicate that an undefined non-terminal was treated as terminal text or that an invalid explicit label selection was ignored.

That mode is useful when you are migrating historical grammars and want visibility into tolerated issues without loosening the tokenizer or parser.

For the fuller warning and checker-parity picture, see [Validation and Diagnostics](/docs/guides/validation-diagnostics).
